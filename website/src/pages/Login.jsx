...
export default function LoginRegister() {
  ...
  // Success callback when Google returns a token or credential
  const onGoogleSuccess = async (tokenResponse) => {
    setError("");
    setLoading(true);
    recordClick("auth_google_attempt", { screen: "login" });

    try {
      // Try backend auth using whatever Google returned
      await handleBackendAuth(tokenResponse);
    } catch (e) {
      console.error("Auth error:", e);
      setError(e?.message || "Sign-in failed. Try again or use a different browser.");
      recordClick("auth_google_failure", { reason: e?.message || "unknown" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const login = useGoogleLogin({
    ...
    flow: "implicit", // keep default implicit token flow (works for access_token / credential)
  });
