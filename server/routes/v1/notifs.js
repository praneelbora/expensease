...
async function sendToUsers(userIds = [], title = 'Jumbl', body = '', data = {}, category = null, opts = {}) {
  ...
  console.info('[sendToUsers] start', {
    channel,
  ...
  if (!Array.isArray(userIds) || userIds.length === 0) {
    console.warn('[sendToUsers] no user IDs provided');
  ...
  try {
    ...
    console.info('[sendToUsers] users fetched', { requested: userIds.length, found: users.length });
    ...
    for (const u of users) {
      try {
        ...
        console.debug('[sendToUsers] preference check', {
          userId: String(u._id),
          wants: !!wants
        });
        ...
        for (const t of [...ios, ...android]) {
          ...
          if (!Expo.isExpoPushToken(t)) {
            ...
            console.debug('[sendToUsers] skipping non-expo token', { userId: String(u._id) });
            continue;
          }
          tokensToSend.push(t);
          tokenToUser[t] = String(u._id);
        ...
        console.error('[sendToUsers] error evaluating preferences for user', String(u._id), innerErr);
        // continue to next user
      }
    }

    console.info('[sendToUsers] tokens filtered', { tokensFound: tokensToSend.length });

    if (!tokensToSend.length) {
      console.info('[sendToUsers] no tokens after filtering preferences');
    ...
    console.info('[sendToUsers] pushNotifications result', {
      tookMs,
    ...
    if (opts.cleanupInvalidTokens !== false) {
      try {
        ...
        console.info('[sendToUsers] tokensWithErrors', { count: distinct.length });
        ...
        if (distinct.length) {
          ...
          console.info('[sendToUsers] cleaning up invalid tokens for users', cleanupSummary);
          ...
          if (ops.length) {
            ...
            console.info('[sendToUsers] cleaned up invalid tokens from user documents', { usersUpdated: ops.length });
          }
        }
      } catch (err) {
        console.error('cleanup invalid tokens error:', err);
      }
    } else {
      console.debug('[sendToUsers] cleanupInvalidTokens disabled by opts');
    }

    return { sent: result.sentCount || 0, attemptedTokens: tokensToSend.length };
  } catch (err) {
    console.error('[sendToUsers] unexpected error:', err);
