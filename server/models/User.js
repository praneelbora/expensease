...
userSchema.statics.shouldReceiveNotification = function (
  ...
  // Simple logger helper (use console.debug so it can be filtered)
  const log = (...args) => {
    try { console.debug('[notify-check]', ...args); } catch (e) { }
  };

  // channel prefs object (may be undefined)
  const chanPrefs = userDoc.notificationPreferences && userDoc.notificationPreferences[ch];
  log('check', { channel: ch, category: cat, chanPrefsExists: !!chanPrefs });

  // 1) channel master switch
  if (chanPrefs && chanPrefs.enabled === false) {
    log('denied: channel master switch is false');
    return false;
  }
  ...
  const evalOverrideChannel = (ovChan) => {
    ...
    if (ovChan.enabled === false) {
      log('override denies via enabled:false');
      return false;
    }
    if (ovChan.mutedUntil && isMutedUntil(ovChan.mutedUntil)) {
      log('override denies via mutedUntil');
      return false;
    }
    if (cat) {
      const v = readCategoryValue(ovChan.categories, cat);
      if (v === false) {
        log('override denies via category=false');
        return false;
      }
      if (v === true) {
        log('override allows via category=true');
        return true;
      }
    }
    return undefined;
  ...
  if (fromFriendId && Array.isArray(userDoc.friendNotificationOverrides)) {
    ...
    if (ov && ov[ch]) {
      ...
      log('friend override present but no explicit decision');
    }
  }
  ...
  if (groupId && Array.isArray(userDoc.groupNotificationOverrides)) {
    ...
    if (ov && ov[ch]) {
      ...
      if (decision === false) return false;
      if (decision === true) return true;
      log('group override present but no explicit decision');
    }
  }
  ...
  if (cat && chanPrefs && chanPrefs.categories) {
    ...
    if (v === false) {
      log('denied: global category false');
      return false;
    }
    if (v === true) {
      log('allowed: global category true');
      return true;
    }
    log('global category has no explicit value');
  }

  // 5) global mutedUntil for channel
  if (chanPrefs && isMutedUntil(chanPrefs.mutedUntil)) {
    log('denied: global mutedUntil active');
    return false;
  }
