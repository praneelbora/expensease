...
export default function FriendDetails() {
  ...
  // helpers
  const currencyCodes = useMemo(() => getAllCurrencyCodes(), []);
  const currencyOptions = useMemo(() => toCurrencyOptions(currencyCodes), [currencyCodes]);
  ...
  const renderExpense = ({ item: exp }) => {
    ...
    const sub = isSettle
      ? (() => {
          ...
          if (!payer || !receiver) return "Settlement";
      ...
      : (exp.groupId?.name || (exp.splits?.length ? "With friend" : "Personal"));
