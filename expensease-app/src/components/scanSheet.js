...
const ScanReceiptSheet = ({ innerRef, onParsed, onClose }) => {
    ...
    const [postUploadChoiceSelected, setPostUploadChoiceSelected] = useState(null); // "personal" | "split" | null

    // NEW: confirm modal state
    const [confirmChoiceVisible, setConfirmChoiceVisible] = useState(false);
    const [pendingChoice, setPendingChoice] = useState(null); // temp store before confirm

    const [itemsCollapsed, setItemsCollapsed] = useState(true); // collapse items initially

    // personal payment methods (for current user)
    const [currentUserPaymentMethods, setCurrentUserPaymentMethods] = useState([]); // normalized {paymentMethodId,label}
    const [selectedPersonalPaymentMethod, setSelectedPersonalPaymentMethod] = useState(null);
    ...
    const uploadImageToServer = async (uri) => {
        ...
        // kick off friend/group fetch right away
        fetchFriendsAndGroups();
        uploadStartRef.current = Date.now();

        // reset post-upload inline choice state (we show the choice inline now)
        setPostUploadChoiceSelected(null);
        setPendingChoice(null);

        // SHOW selection stage IMMEDIATELY so user can pick Personal / Split
        // parsing will continue in background
        setStage("select");
        ...
        try {
            ...
            // immediate parsed result if server returned it synchronously
            const immediateParsed = data.parsed ?? (data.items || data.rawText ? data : null);
            if (immediateParsed) {
                console.log('immediateParsed: ');

                setServerParsed(immediateParsed);
                const items = Array.isArray(immediateParsed.items) ? immediateParsed.items : [];
            ...
            // if server returned only rawText or something similar
            if (data.rawText) {
                ...
                setTimeout(() => {
                    ...
                    setUploading(false);
                    // we intentionally don't force navigation here — keep select open so user can continue
                }, wait2);
                return;
            }
            ...
            // do NOT forcibly setUploading(false) here, success branch already does that
        }
    };



    const applyPostUploadChoice = async () => {
        const choice = pendingChoice;
        if (!choice) {
            // nothing selected — keep user on select stage
            setStage("select");
            return;
        }

        // persist selection
        setPostUploadChoiceSelected(choice);
        setSelectionConfirmed(true);

        // reset pendingChoice so subsequent flows use postUploadChoiceSelected
        setPendingChoice(null);

        if (choice === "split") {
            // go to friend/group selection stage
            setStage("picking");
            return;
        }
        ...
        setConfirmedParticipants(me);
        assignAllItemsToParticipants();

        try {
            const uid = currentUser?._id;
            if (uid) {
                const map = await fetchFriendsPaymentMethods([uid], user?.token);
                const raw = map[uid] || [];
                const normalized = raw.map(pm => ({
                    paymentMethodId: pm.paymentMethodId || pm.id || pm._id || pm.token,
                    label: pm.label || pm.nickname || pm.type || `•••• ${pm.last4 || ""}`,
                }));
                setCurrentUserPaymentMethods(normalized);

                if (normalized.length === 1) {
                    setSelectedPersonalPaymentMethod(normalized[0].paymentMethodId);
                } else {
                    setSelectedPersonalPaymentMethod(null);
                }
                ...
                setConfirmedParticipants(prev =>
                    prev.map(p => ({
                        ...
                        selectedPaymentMethodId: normalized.length === 1 ? normalized[0].paymentMethodId : null,
                    }))
                );
            }
        } catch (e) {
            console.warn("fetch PMs for personal failed", e);
        }

        // move to parsed if available, otherwise show loading stage

        console.log('serverParsed: ', serverParsed);
        setStage("parsed");
    };
    ...
    const onUseParsedWithConfirm = () => {
        // Validate selectionConfirmed for split (we need a friend/group) OR personal (we should have 'personal' selected)
        if (!selectionConfirmed && postUploadChoiceSelected !== "personal") {
            Alert.alert("Confirm selection", "Please confirm the friend or group selection first.");
            return;
        }

        // For split mode: ensure every item assigned to at least one participant
        if (postUploadChoiceSelected === "split" && !allItemsAssigned) {
            Alert.alert("Assign items", "Each line item must be assigned to at least one person before saving.");
            return;
        }

        // For personal mode: ensure payment method is chosen if there are multiple methods
        if (postUploadChoiceSelected === "personal") {
            if (currentUserPaymentMethods.length > 1 && !selectedPersonalPaymentMethod) {
                Alert.alert("Choose payment method", "Please select a payment account for this personal expense.");
                return;
            }
            // items should be assigned to current user automatically; still warn if no items exist? optional
        }
    ...
    const FriendRow = ({ f }) => {
        ...
        return (
            <TouchableOpacity
                onPress={() => { onSelectFriendSingle(f); }}
    ...
    const finalizeAndClose = async () => {
        ...
        try {
            ...
            // shared receipt metadata if present
            const receiptObj = receiptMeta
                ? {
                    receipt: {
                        receiptId: receiptMeta.receiptId ?? null,
                        storage: receiptMeta.file ? "s3" : null,
                        file: receiptMeta.file ?? null,
            ...
            // Personal mode
            if (postUploadChoiceSelected === "personal") {
                // ensure payment method selected if required
                const pmForPayload = selectedPersonalPaymentMethod || (currentUserPaymentMethods.length === 1 ? currentUserPaymentMethods[0].paymentMethodId : null);
                ...
                const payload = {
                    ...
                    currency: serverParsed?.currency || "INR",
                    paymentMethodId: pmForPayload || undefined,
                    ...receiptObj,
                };

                // if no payment method selected but user had multiple, prevent save
                if (currentUserPaymentMethods.length > 1 && !pmForPayload) {
                    Alert.alert("Choose payment method", "Please choose a payment method for this personal expense.");
                    setSaving(false);
                    return;
                }
                ...
                // Build participant splits from current UI state
            ...
            // Success UX
            try { innerRef?.current?.dismiss?.(); } catch (e) { }
            resetAll();
    ...
    const renderFooter = ({ busy, primaryDisabled, defaultLayout } = {}) => {
        ...
        if (stage === "select") {
            return (
                <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
                    <View style={{ flex: 1 }} />
                    <TouchableOpacity
                        style={[styles.actionBtnSmall, (!pendingChoice) && styles.actionBtnDisabled]}
                        disabled={!pendingChoice}
                        onPress={() => applyPostUploadChoice()}
                    >
        ...
        // Parsed footer (handles both split and personal)
        if (stage === "parsed") {
            if (postUploadChoiceSelected === "personal") {
                // personal rules: need items assigned (optional), and payment method chosen if user has >1
                const needPM = currentUserPaymentMethods.length > 1 && !selectedPersonalPaymentMethod;
                const noItems = editingItems.length === 0;
                const disableSave = needPM || noItems || saving;

                let helperText = "";
                if (needPM) helperText = "Choose payment method";
                else if (noItems) helperText = "No items detected";
                else helperText = "Save Expense";
                    ...
                    <View style={{ flexDirection: "row", gap: 10, justifyContent: "flex-end" }}>
                        ...
                        >
                            <Text style={{ color: "#fff", fontWeight: "700", paddingHorizontal: 16 }}>
                                {needPM ? "Choose payment method" : "Save Expense"}
                            </Text>
                        </TouchableOpacity>
                    </View>
                );
            ...
            // default: split mode behavior (existing)
            const disableSave = !allItemsAssigned || paidHasIssue || payersArr.length === 0 || pmMissingForActivePayer || saving;
            return (
                <View style={{ flexDirection: "row", gap: 10, justifyContent: "flex-end" }}>
                    <TouchableOpacity
                        style={[styles.actionBtn, disableSave && styles.actionBtnDisabled]}
    ...
    return (
        <BottomSheetLayout
            ...
            hideFooter={false}
        >
            <KeyboardAwareScrollView style={styles.container} contentContainerStyle={{ paddingBottom: insets.bottom + 24, backgroundColor: colors?.background }} keyboardShouldPersistTaps="handled">
                {/* Choose (unchanged) */}
                    ...
                    <View style={{ marginTop: 20 }}>
                        <View style={styles.selectCard}>
                            <View style={styles.selectHeader}>
                                {localImage ? (
                                    <Image source={{ uri: localImage.uri }} style={styles.selectThumb} resizeMode="cover" />
                                ) : (
                                    <View style={styles.selectThumbPlaceholder}>
                                        <Text style={styles.selectThumbPlaceholderText}>No image</Text>
                                    </View>
                                )}

                                <View style={{ flex: 1, marginLeft: 12 }}>
                                    <Text numberOfLines={1} style={styles.selectMerchant}>
                                        {serverParsed?.merchant?.name ?? serverParsed?.description ?? "Receipt"}
                                    </Text>
                                    {serverParsed && (serverParsed?.date || serverParsed?.currency) && <Text style={styles.selectMeta}>
                                        {serverParsed?.date ? new Date(serverParsed.date).toLocaleDateString() : ""}
                                        {serverParsed?.currency || serverParsed?.total ? ` • ${serverParsed?.currency ?? "INR"} ${formatCurrency(serverParsed?.total ?? subtotal())}` : ""}
                                    </Text>}
                                    <Text style={styles.helperSmall} numberOfLines={2}>
                                        Choose whether this expense is a personal spend or a split with friends.
                                    </Text>
                                </View>
                            </View>
                            <View style={{ flex: 1, flexDirection: "row", marginTop: 14, justifyContent: 'space-between' }}>
                                <TouchableOpacity
                                    activeOpacity={0.85}
                                    onPress={() => { setPendingChoice("personal"); Keyboard?.dismiss?.(); }}
                                    style={[
                                        styles.optionCard,
                                        pendingChoice === "personal" && styles.optionCardActive
                                    ]}
                                    accessibilityRole="button"
                                    accessibilityLabel="Record as personal expense"
                                >
                                    <View style={{ flex: 1 }}>
                                        <Text style={[styles.optionTitle, pendingChoice === "personal" && styles.optionTitleActive]}>Personal</Text>
                                    </View>
                                    {pendingChoice === "personal" && <View style={styles.optionCheck}><Text style={styles.optionCheckTick}>✓</Text></View>}
                                </TouchableOpacity>

                                <TouchableOpacity
                                    activeOpacity={0.85}
                                    onPress={() => { setPendingChoice("split"); Keyboard?.dismiss?.(); }}
                                    style={[
                                        styles.optionCard,
                                        pendingChoice === "split" && styles.optionCardActive
                                    ]}
                                    accessibilityRole="button"
                                    accessibilityLabel="Record as split expense"
                                >
                                    <View style={{ flex: 1 }}>
                                        <Text style={[styles.optionTitle, pendingChoice === "split" && styles.optionTitleActive]}>Split</Text>
                                    </View>
                                    {pendingChoice === "split" && <View style={styles.optionCheck}><Text style={styles.optionCheckTick}>✓</Text></View>}
                                </TouchableOpacity>
                            </View>

                            {/* Beta warning (compact) */}
                            <View style={styles.betaRow}>
                                <View style={styles.betaIcon}><Text style={{ fontWeight: "800" }}>⚠️</Text></View>
                                <View style={{ flex: 1 }}>
                                    <Text style={styles.betaTitle}>Receipt Scan (Beta)</Text>
                                    <Text style={styles.betaText}>
                                        Parsed items, tax or totals may be incorrect. Please review before saving.
                                    </Text>

                                    <View style={{ flexDirection: "row", marginTop: 8 }}>
                                        <TouchableOpacity
                                            onPress={() => {
                                                const email = 'email.expensease@gmail.com';
                                                const subject = encodeURIComponent('Feedback: Receipt Scan Beta');
                                                const body = encodeURIComponent('I want to share feedback about receipt scanning:');
                    ...
                    <View style={{ marginTop: 12 }}>
                        ...
                        <View style={{ marginTop: 8 }}>
                            {filteredGroups.length === 0 ? (
                                ...
                                // simple map instead of FlatList
                                filteredGroups.map((g) => (
                                    <View key={g._id || g.id || g.name} style={{ marginBottom: 8 }}>
                                        <GroupRow g={g} />
                                    </View>
                                ))
                            )}
                        ...
                        <View style={{ marginTop: 8 }}>
                            {filteredFriends.length === 0 ? (
                                ...
                                filteredFriends.map((f) => (
                                    <View key={f._id || f.id || f.name} style={{ marginBottom: 8 }}>
                                        <FriendRow f={f} />
                                    </View>
                                ))
                            )}
                            ...
                            <>
                                ...
                                <View style={styles.summaryCard}>
                                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                                        <Text style={{ fontSize: 16, fontWeight: "700", color: colors.text }}>{serverParsed?.merchant?.name ?? serverParsed?.description ?? "Receipt"}</Text>
                                        <View style={{ flexDirection: "column", justifyContent: "", alignItems: "flex-end" }}>
                                            ...
                                            <Text style={{ textTransform: 'capitalize', fontSize: 12, color: colors.muted ?? "#666" }}>{postUploadChoiceSelected} expense</Text>
                                        </View>
                                    </View>
                                ...
                                <View style={{ marginTop: 12 }}>
                                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                                        ...
                                        <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
                                            <TouchableOpacity style={styles.smallBtn} onPress={() => setItemsCollapsed(v => !v)}>
                                                <Text style={styles.btnText}>{itemsCollapsed ? `View items (${editingItems.length})` : "Hide items"}</Text>
                                            </TouchableOpacity>
                                            {/* If personal, show a small badge explaining items are assigned to you */}
                                        </View>
                                    </View>
                                                    ...
                                                    return (
                                                        <View key={it.id} style={styles.itemCard}>
                                                            <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" }}>
                                                                ...
                                                                <View style={{ width: postUploadChoiceSelected === "split" ? 150 : 50, alignItems: "flex-end", flexDirection: 'column', gap: 2 }}>
                                                                    {postUploadChoiceSelected === "split" && (
                                ...
                                <View style={{ marginTop: 6, alignItems: "flex-end" }}>
                                    <TouchableOpacity onPress={() => setEditTotalsOpen((v) => !v)} style={[styles.neutralBtn]}>
                                        <Text style={styles.pillText}>{editTotalsOpen ? "Hide Editor" : "Edit Summary"}</Text>
                                    </TouchableOpacity>
                                </View>
                                ...
                                {postUploadChoiceSelected == "split" && <View style={{ paddingTop: 8, borderTopWidth: 1, borderTopColor: colors.border || "#eee", }}>
                                ...
                                {/* Personal payment method selector */}
                                {postUploadChoiceSelected === "personal" && (
                                    <View style={{ marginTop: 12, borderTopWidth: 1, borderTopColor: colors.border || "#eee", paddingTop: 12 }}>
                                        <Text style={{ fontSize: 13, fontWeight: "700", color: colors.text }}>Payment account</Text>
                                        <View style={{ marginTop: 8 }}>
                                            {currentUserPaymentMethods.length === 0 ? (
                                                <Text style={styles.helperSmall}>No payment methods saved.</Text>
                                            ) : currentUserPaymentMethods.length === 1 ? (
                                                <View style={{ marginTop: 6 }}>
                                                    <Text style={{ fontWeight: "600" }}>Using: {currentUserPaymentMethods[0].label}</Text>
                                                </View>
                                            ) : (
                                                <View style={{ marginTop: 8 }}>
                                                    <Dropdown
                                                        value={selectedPersonalPaymentMethod || undefined}
                                                        onChange={(val) => setSelectedPersonalPaymentMethod(val)}
                                                        options={currentUserPaymentMethods.map(pm => ({ value: pm.paymentMethodId, label: pm.label }))}
                                                        placeholder="Choose payment method…"
                                                        align="right"
                                                    />
                                                    <Text style={[styles.helperSmall, { marginTop: 8 }]}>Select which account to charge for this personal expense.</Text>
                                                </View>
                                            )}
                                        </View>
                                    </View>
                                ...
                                {postUploadChoiceSelected == "split" && <View style={{ marginTop: 8, borderTopWidth: 1, borderTopColor: colors.border || "#eee" }}>
                                ...
                                {/* Who paid */}
                                {postUploadChoiceSelected == "split" && <View style={{ marginTop: 12, borderTopWidth: 1, borderTopColor: colors.border || "#eee", paddingTop: 12 }}>
                                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <Modal
                visible={confirmChoiceVisible}
                transparent
                animationType="fade"
                onRequestClose={() => setConfirmChoiceVisible(false)}
            >
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalCard, { maxWidth: 420 }]}>
                        <Text style={{ fontSize: 18, fontWeight: "800", color: colors.text }}>Confirm choice</Text>
                        <Text style={[styles.helperSmall, { marginTop: 8 }]}>
                            This choice can’t be changed later. Are you sure?
                        </Text>

                        <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}>
                            <TouchableOpacity
                                style={[styles.smallBtn]}
                                onPress={() => setConfirmChoiceVisible(false)}
                            >
                            ...
                            <TouchableOpacity
                                ...
                                onPress={applyPostUploadChoice}
                            >
                                <Text style={styles.btnTextPrimary}>Yes, continue</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
...
const createStyles = (colors = {}, theme) =>
    StyleSheet.create({
        ...
        smallBtn: { paddingVertical: 10, paddingHorizontal: 12, borderWidth: 1, borderRadius: 8, backgroundColor: colors.cardMid ?? colors?.cardAlt ?? "#f4f4f4", borderColor: colors?.border },
        ...
        modalOverlay2: { flex: 1, justifyContent: "center", alignItems: "center", padding: 20 },
        modalCard: { width: "100%", maxWidth: 680, backgroundColor: colors.background, borderRadius: 12, padding: 16 },
        dropdownTrigger: {
        ...
        selectThumb: { width: 72, height: 72, borderRadius: 8, backgroundColor: "#f4f4f4" },
        selectThumbPlaceholder: { width: 72, height: 72, borderRadius: 8, backgroundColor: colors.input ?? "#f4f4f4", alignItems: "center", justifyContent: "center" },
        selectThumbPlaceholderText: { color: colors.muted ?? "#888", fontSize: 11 },
        selectMerchant: { fontSize: 15, fontWeight: "800", color: colors.text, marginBottom: 4 },
        selectMeta: { fontSize: 12, color: colors.muted ?? "#666" },
        ...
        optionCardActive: {
            ...
            borderColor: colors.cta ?? colors.primary ?? "#00C49F",
            shadowColor: "#000",
        ...
        optionSubtitle: { fontSize: 12, color: colors.muted ?? "#777", marginTop: 2 },

        optionCheck: { marginLeft: 8, width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(255,255,255,0.12)" },
        optionCheckTick: { color: "#fff", fontWeight: "800", fontSize: 14 },

        betaRow: {
            marginTop: 14,
            padding: 12,
            borderRadius: 10,
            backgroundColor: theme?.mode === 'dark' ? '#3a2c13' : '#fff7e6',
            borderWidth: 1,
            borderColor: theme?.mode === 'dark' ? '#5a431e' : '#ffe8a8',
            flexDirection: 'row',
            gap: 10,
            alignItems: 'flex-start',
        },
        betaIcon: { width: 28, height: 28, borderRadius: 18, backgroundColor: '#ffd54f', alignItems: 'center', justifyContent: 'center', marginRight: 8 },
        betaTitle: { fontWeight: "700", color: colors.text },
        betaText: { fontSize: 12, color: colors.muted ?? "#666", marginTop: 4 },

        feedbackBtn: { marginTop: 4, paddingVertical: 6, paddingHorizontal: 10, borderRadius: 10, borderWidth: 1, borderColor: colors.text, alignSelf: "flex-start" },
        feedbackBtnText: { fontSize: 12, fontWeight: "700", color: colors.text },

        continueBtn: { backgroundColor: colors.cta ?? colors.primary ?? "#00C49F", paddingVertical: 10, paddingHorizontal: 16, borderRadius: 10, alignItems: "center", justifyContent: "center" },
